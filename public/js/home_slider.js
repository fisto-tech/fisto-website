...
class Slideshow {
  ...
  animate(direction = "next") {
    ...
    (this.DOM.activeSlide = this.DOM.el.querySelector(".swiper-slide-active")),
      (this.DOM.activeSlideImg =
        this.DOM.activeSlide.querySelector(".slide-image")),
      (this.DOM.activeSlideTitle =
        this.DOM.activeSlide.querySelector(".slide-title")),
    ...
    if (this.DOM.oldSlide) {
      // Get parts
      (this.DOM.oldSlideTitle =
        this.DOM.oldSlide.querySelector(".slide-title")),
    ...
    // Animate paragraph (bottom to top)
    const slidePara = this.DOM.activeSlide.querySelector(".slide-para");
    if (slidePara) {
      TweenMax.fromTo(
        slidePara,
        1.2,
        { x: -200, opacity: 0 },
        { x: 0, opacity: 1, ease: Power3.easeOut, delay: 0.4 }
      );
    }

    // Animate image (bottom to top)
    const slideImage = this.DOM.activeSlide.querySelector(".slide-img img");
    if (slideImage) {
      TweenMax.fromTo(
        slideImage,
        1.5,
        { y: 150, opacity: 0 },
        { y: 0, opacity: 1, ease: Expo.easeOut, delay: 0.5 }
      );
    }
  ...
  animatePagination(swiper, paginationEl) {
    ...
    this.DOM.activePaginationItemLoader =
      this.DOM.activePaginationItem.querySelector(
        ".pagination-separator-loader"
      );
